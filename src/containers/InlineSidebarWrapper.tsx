...
const InlineSidebarWrapper: FC<PropsWithChildren> = ({ children }) => {
  ...
  return (
    <div className='relative z-10 mb-8 block lg:hidden' ref={inlineSidebarRef}>
      <button
        className={`flex h-10 w-full items-center gap-2 rounded-md bg-bgDark-200 px-3 text-lg font-medium ${
          showChildren ? 'dark:bg-bgDark-700' : 'dark:bg-bgDark-800'
        }`}
      ...
      {showChildren && (
        <div className='absolute top-4 -z-10 w-full rounded-b-md bg-bgDark-100 px-5 pt-10 pb-5 dark:bg-bgDark-700'>
          {children}
        </div>
      )}
    </div>
